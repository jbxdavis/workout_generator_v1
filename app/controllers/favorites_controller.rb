class FavoritesController < ApplicationController
  before_action :current_user_must_be_favorite_user, :only => [:show, :edit_form, :update_row, :destroy_row]
  ...
  def index
    ...
    @favorites = @q.result(:distinct => true).includes(:user, :lift).page(params[:page]).per(10)
  ...
  def create_row
    ...
    @favorite.lift_id = params.fetch("lift_id")
  ...
  def create_row_from_lift
    ...
    if @favorite.valid?
      ...
      redirect_to("/lifts/#{@favorite.lift_id}", notice: "Favorite created successfully.")
    else
      render("favorite_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_lift
    ...
    redirect_to("/lifts/#{@favorite.lift_id}", notice: "Favorite deleted successfully.")
  end
